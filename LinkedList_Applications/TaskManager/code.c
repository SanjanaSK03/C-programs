...
void displayTasks(struct task *head, FILE *output);
void removeTask(struct task **head, char n[], FILE *output);
void totalTasks(struct task *head, FILE *output);
...
int main()
{
    ...
    while (fscanf(input, "%s", x) != EOF)
    {
        ...
        {
            fscanf(input, "%s", n);
            removeTask(&head, n, output);
        }
        else if (!strcmp(x, "display"))
        {
            fprintf(output, "Task List:\n");
            displayTasks(head, output);
        }
